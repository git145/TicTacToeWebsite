 ...
 <html lang = "en">
     ...
     <!--
         File: tictactoe_hoftwon.php
         Module: COMP519
         Author: Richard Kneale
         Student ID: 200790336
         Date created: 1st January 2016
         Description: Displays the three users who have won the most tournaments
       -->
     ...
     <head> 
		 ...
		 <?php
			 ...
			 else
			 {
				 ...
				 $sql = "SELECT * FROM ".$table." ORDER BY tWon DESC";
				 $result = $db->query($sql);
				 ...
				 $user1 = $row[$USERNAME];
				 $tWon1 = $row[$TOURNAMENTS_WON];
				 
				 // Get data for first player
				 $result->data_seek(1);
				 $row = $result->fetch_row();
				 $user2 = $row[$USERNAME];
				 $tWon2 = $row[$TOURNAMENTS_WON];
				 
				 // Get data for first player
				 $result->data_seek(2);
				 $row = $result->fetch_row();
				 $user3 = $row[$USERNAME];
				 $tWon3 = $row[$TOURNAMENTS_WON];
     ...
     <body>
         <?php
			 ...
			 print(  '<div id = "resultsSpace">
						 <div class = "textCenter boldText largeText" id = "hallOfFameTitle">Top 3 Players in Tournaments Won</div><br/>
						 
						 <table id = "hallOfFameTable">
							 <tr>
								 <td class = "textLeft boldText">'.$user1.':</td>
								 <td class = "textCenter" id = "numberOfGames">'.$tWon1.'</td>
							 </tr>
             
							 <tr>
								 <td class = "textLeft boldText">'.$user2.':</td>
								 <td class = "textCenter" id = "numberOfGames">'.$tWon2.'</td>
							 </tr>
             
							 <tr>
								 <td class = "textLeft boldText">'.$user3.':</td>
								 <td class = "textCenter" id = "numberOfGames">'.$tWon3.'</td>
							 </tr>
						 </table><br/>
						 '.getHOFButtons().'
					 </div>'
